(function () {
  ...
  const STORAGE_KEY = "myRoster.override.v1";
  const DEFAULT_LABEL = "Trascina qui un nuovo file .xls / .xlsx per sostituire i turni precaricati";
  ...
  function loadDefaultEntries() {
    ...
    entries = source.map((e) => ({ ...e, date: new Date(e.date) }));
    els.fileDropLabel.textContent = DEFAULT_LABEL;
  ...
  function loadFromStorage() {
    ...
    try {
      ...
      els.fileDropLabel.textContent = payload.fileName
        ? `File caricato: ${payload.fileName} (carica un nuovo file per sostituirlo)`
        : "File personalizzato caricato.";
